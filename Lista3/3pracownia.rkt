...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;CAR PRZED CDR< ZEBY DZIAŁAŁO W INVERTED_MODULO

(define (invert-modulo e m)
  (if (= (gcd e m) 1)
      (let ((y (car (cdr (solve-ax+by=1 m e)))));;;;o tutaj
...
(require math)
(define (solve-ax+by=1 a b) (bezout a b))
...
(define (RSA-unconvert-list intlist key)
    ...
    (define (convert l sum)
      (cond
        [(null? l) '()]
        [else  (let ((x (modulo (+ (RSA-transform (car l) key) sum) n)))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
 (let ((encrypted
       ...
       (let ((signed
              (RSA-convert-list (cons (compress encrypted) null) sender-private-key)))
   (make-signed-message encrypted signed))))
  ;;; XXX: Write your code here
  ;;(error "Not implemented"))

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
(let ({decrypted
         (car (RSA-unconvert-list (signature cyphertext) sender-public-key))})
    (cond
      [(= decrypted (compress (message cyphertext))) (RSA-decrypt (message cyphertext) recipient-private-key)]
      [else (error "Incorrect signature !!!")])))
  ;;; XXX: Write your code here
  ;;(error "Not implemented"))
...
;(authenticate-and-decrypt '((13877480 813203301 743213754) 338955505
;                                                             ) (key-pair-private test-key-pair1) (key-pair-public test-key-pair2) )

;(encrypt-and-sign "lubie placki" (key-pair-private test-key-pair1) (key-pair-public test-key-pair2))
;(encrypt-and-sign "DZIEKUJE"
;                  (key-pair-private test-key-pair1) (key-pair-public test-key-pair2))
;(authenticate-and-decrypt '((52670268 91024930) 80631385) (key-pair-public test-key-pair1) (key-pair-private test-key-pair2) )
