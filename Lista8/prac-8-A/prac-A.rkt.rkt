...
;(define y (list ((quote +) 1 (quote +) 2 3)))

(eval '(let (x (lambda-rec (evall expr)
                    (cond
                      [(pair? expr)
                          (cond
                            [(eq? (car expr) (quote +))
                                (+ (evall (car (cdr expr))) (evall (car (cdr (cdr expr)))))]
                            [(eq? (car expr) (quote -))
                                 (- (evall (car (cdr expr))) (evall (car (cdr (cdr expr)))))]
                            [(eq? (car expr) (quote *))
                                 (* (evall (car (cdr expr))) (evall (car (cdr (cdr expr)))))]
                            [(eq? (car expr) (quote /))
                                (/ (evall (car (cdr expr))) (evall (car (cdr (cdr expr)))))])]
                      [true ;nie mamy else a true działa tak samo
                          expr])))
  (x (list (quote +) 5 (list (quote +) 1 2)))))

#|
(define ev '(lambda-rec (evall expr)
                    (cond
                      [(pair? expr)
                          (cond
                            [(eq? (car expr) (quote +))
                                (+ (ev (car (cdr expr))) (ev (car (cdr (cdr expr)))))]
                            [(eq? (car expr) (quote -))
                                 (- (ev (car (cdr expr))) (ev (car (cdr (cdr expr)))))]
                            [(eq? (car expr) (quote *))
                                 (* (ev (car (cdr expr))) (ev (car (cdr (cdr expr)))))]
...
(define test1 (ev '(list (quote +) 1 3))) |#
 ; (x (list (quote +) 1 3))))

;(cons (quote +) (cons 1 (cons 3 null)))


; do końcówki potrzebujemy zdefiniowanych list żeby nie pisać w przykładach miliona par