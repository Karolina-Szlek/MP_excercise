...
(define (eval-env e env)
  (cond [(const? e)
        ...
        [(list?? e);;Dopisiane, fragment rozwiązania pracowni
         (if (null? (cdr e))
             e
             (cons (eval-env (car (cdr e)) env)
                   (eval-env (cons 'list (cdr (cdr e))) env)))]
        [(lambda? e)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;napisane podobnie jak ćw.2 z zajęć

(define (list?? e);;dodane do ewaluatora pomocnicze, żeby nie trzeba było robić wielu cons-ów 
  (cond [(and (list? e) (eq? (car e) 'list)) true]
      [(not (and (list? e) (eq? (car e) 'list))) false]))

;TEST 1 (z opisem)
(display "TEST 1")
(newline)
;dla operacji binarnych muszę określić co dla (+ - * /)
(eval '(let (operation (lambda-rec (eval-l exp)
                    (cond
                      [(pair? exp)
                          (cond
                            [(eq? (quote +) (car exp));dla +
                                (+ (eval-l (car (cdr (cdr exp)))) (eval-l (car (cdr exp))))]
                            [(eq? (quote -) (car exp));dla -
                                 (- (eval-l (car (cdr exp))) (eval-l (car (cdr (cdr exp)))))]
                            [(eq? (quote *) (car exp));dla *
                                 (* (eval-l (car (cdr (cdr exp)))) (eval-l (car (cdr exp))))]
                            [(eq? (quote /) (car exp));dla /
                                (/ (eval-l (car (cdr exp))) (eval-l (car (cdr (cdr exp)))))])]
                      [true exp])));nie zdefiniowałam else, ale true zadziała tak samo
  (operation (list (quote -) (list (quote *) 9 2) 5))));<== tu podajemy wyrażenie
...
(eval '(let (operation (lambda-rec (eval-l exp)
                    (cond
                      [(pair? exp);czy dostaję listę (parę )
                          (cond
                            [(eq? (quote +) (car exp));dla +
                                (+ (eval-l (car (cdr (cdr exp)))) (eval-l (car (cdr exp))))]
                            [(eq? (quote -) (car exp));dla -
                            ...
                            [(eq? (quote /) (car exp));dla /
                                (/ (eval-l (car (cdr exp))) (eval-l (car (cdr (cdr exp)))))])]
                      [true exp])));nie zdefiniowałam else, ale true zadziała tak samo
  (operation (list (quote *) (list (quote -) 9 2) (list (quote /) 18 3)))))
                      ...
                      [(pair? exp);czy dostaję listę (parę )
                          (cond
                            ...
                            [(eq? (quote /) (car exp));dla /
                                (/ (eval-l (car (cdr exp))) (eval-l (car (cdr (cdr exp)))))])]
                      [true exp])));nie zdefiniowałam else, ale true zadziała tak samo
  (operation (list (quote *) (list (quote -) 9 (list (quote +) 7 1)) (list (quote /) 18 3)))))
...
;;Brałam udział w pierwszych 15 min zajęć prowadzonych przez Mateusza Kacałe w KSI.