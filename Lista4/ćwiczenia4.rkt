...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;ĆWICZENIA;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;zad 1 omówione tylko , ogarnij od innych




;;zad 2 concatMap f xs ->do każdego x stosuje f  i powstaje listalist
;;(jak w map) ale ty, teraz je łączy w jedną listę 
...
;;każdemu el tablicy nakłada na 1-size , qcalls=k-1

;;ten już zaimplementowany jest szybszy bo bierze pozycje które ma i do każdej dodaje kolejną
;;a nowy kod dla każdej nowej pozycji generuje na nowo stare 


;;fragment do zastąpienia
;(concatMap1
...
;;Drzewo przeszukiwań binarne ma zawsze 2 dzieci
;;Drzewo przeszukiwań binarnych => mniejsze większe
...
(define (mirror tree)
  (if (leaf? tree)
      leaf
      (make-node (node-val tree) (mirror (node-right tree)) (mirror (node-left tree)))))
...
(flatten1 '(node a (node b (node c leaf leaf) leaf) (node d leaf leaf)))
...
(define (flatten tree)
  (define (helper tree acc)
    (if (leaf? tree)
        ...
        (helper (node-left tree) ( cons node-val ( helper (node-right tree) acc)))))
  (helper tree 0))
...
(define (bst-insert1 x t)
  (cond [(leaf? t)
         (make-node x leaf leaf)]
        [(< x (node-val t))
         (make-node (node-val t)
                    (bst-insert1 x (node-left t))
                    (node-right t))]
        [else
         (make-node (node-val t)
                    (node-left t)
                    (bst-insert1 x (node-right t)))]))



;;zad 5
;(define (treesort list)
;(flatten (fold bts-insert leaf list)
;;fold , sprawdż jak działa 
         
; rób drzebo bts ,flaten,   
